...
#include "SVG.h"
	...
	bool PolygonProximityLinker::isLinked(curaIrfan::PointIrfan from)
	{
		return point_to_link.find(from) != point_to_link.end();
	}


	std::pair<PolygonProximityLinker::Point2Link::iterator, PolygonProximityLinker::Point2Link::iterator> PolygonProximityLinker::getLinks(curaIrfan::PointIrfan from)
	{
		...
		for (Point2Link::iterator it = from_link_pair.first; it != from_link_pair.second; ++it)
		{
			if (!(it->second.a.p() == from || it->second.b.p() == from))
			{
				...
				std::cerr << std::hash<curaIrfan::PointIrfan>()(from) << " hashes " << std::hash<curaIrfan::PointIrfan>()(it->second.a.p()) << " or " << std::hash<curaIrfan::PointIrfan>()(it->second.b.p()) << "\n";
				//             std::cerr << "ERROR! some point got mapped to a link which doesn't have the point as one of the end points!\n";

				std::cerr << "\n all links:\n";
				for (std::pair<const curaIrfan::PointIrfan, const ProximityPointLink> pair : point_to_link)
				{
	...
	void PolygonProximityLinker::findProximatePoints()
	{
		...
		for (const ListPolyIt& new_point_it : new_points)
		{
			...
			{
				curaIrfan::PointIrfan new_point = new_point_it.p();
				curaIrfan::PointIrfan nearby_vert = nearby_vert_it.p();
				int64_t dist2 = curaIrfan::vSize2(curaIrfan::operator-(new_point , nearby_vert));
	...
	{
		const curaIrfan::PointIrfan& a_point = a_point_it.p();

		const curaIrfan::PointIrfan& b_from = b_from_it.p();
		const curaIrfan::PointIrfan& b_to = b_to_it.p();
		...
		if (dist2 > proximity_distance_2
			|| (a_point_it.poly == &to_list_poly
				&& curaIrfan::dot(curaIrfan::operator-(a_point_it.next().p(), a_point), curaIrfan::operator-(b_to, b_from)) > 0	  
				&& curaIrfan::dot(curaIrfan::operator-(a_point, a_point_it.prev().p()), curaIrfan::operator-(b_to, b_from)) > 0) // line segments are likely connected, because the winding order is in the same general direction
			)
		...
		if (curaIrfan::shorterThen(curaIrfan::operator-(closest , b_from), 10))
		{
			addProximityLink(a_point_it, b_from_it, dist, ProximityPointLinkType::NORMAL);
		}
		else if (curaIrfan::shorterThen(curaIrfan::operator-(closest , b_to), 10))
		{
	...
	{
		curaIrfan::PointIrfan& a1 = link.a.p();
		curaIrfan::PointIrfan& a2 = a2_it.p();
		curaIrfan::PointIrfan& b1 = link.b.p();
		curaIrfan::PointIrfan& b2 = b2_it.p();
		curaIrfan::PointIrfan a = curaIrfan::operator-(a2 , a1);
		curaIrfan::PointIrfan b = curaIrfan::operator-(b2 , b1);
		...
		if (dist < 0) { return; }
		int64_t a_length2 = curaIrfan::vSize2(a);
		int64_t b_length2 = curaIrfan::vSize2(b);
	...
	//         DEBUG_PRINTLN("Next point should have been linked already!!");
			...
			{
				curaIrfan::PointIrfan b_p = curaIrfan::operator+(b1 , curaIrfan::normal(b, dist));
			...
			{
				curaIrfan::PointIrfan a_p = curaIrfan::operator+(a1 , curaIrfan::normal(a, dist));
	...
	ListPolyIt PolygonProximityLinker::addNewPolyPoint(const curaIrfan::PointIrfan point, const ListPolyIt line_start, const ListPolyIt line_end, const ListPolyIt before_this)
	{
	...
	int64_t PolygonProximityLinker::proximityEndingDistance(curaIrfan::PointIrfan& a1, curaIrfan::PointIrfan& a2, curaIrfan::PointIrfan& b1, curaIrfan::PointIrfan& b2, int a1b1_dist)
	{
		int overlap = proximity_distance - a1b1_dist;
		curaIrfan::PointIrfan a = curaIrfan::operator-(a2 , a1);
		curaIrfan::PointIrfan b = curaIrfan::operator-(b2 , b1);
		double cos_angle = INT2MM2(curaIrfan::dot(a, b)) / curaIrfan::vSizeMM(a) / curaIrfan::vSizeMM(b);
		...
		{
			return std::min(curaIrfan::vSize(b), curaIrfan::vSize(a));
		}
		else
		{
	...
	void PolygonProximityLinker::addToPoint2LinkMap(curaIrfan::PointIrfan p, ProximityPointLinks::iterator it)
	{
	...
	void PolygonProximityLinker::proximity2HTML(const char* filename) const
	{
		...
		SVG svg(filename, aabb, curaIrfan::PointIrfan(1024 * 2, 1024 * 2));
		...
		{ // output points and coords
			...
			{
				for (curaIrfan::PointIrfan& p : poly)
				{
		...
		{ // output links
			...
			for (const ProximityPointLink& link : copy.proximity_point_links)
			{
				...
				curaIrfan::PointIrfan a = svg.transform(link.a.p());
				curaIrfan::PointIrfan b = svg.transform(link.b.p());
