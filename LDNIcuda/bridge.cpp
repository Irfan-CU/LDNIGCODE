...
#include "Polygon.h"
	...
	int bridgeAngle(const Polygons& skin_outline, const SliceDataStorage& storage, const unsigned layer_nr, const SupportLayer* support_layer, Polygons& supported_regions)
	{
		...
		for (const SliceMeshStorage& mesh : storage.meshes)
		{
			if (mesh.isPrinted())
			{
				for (const SliceLayerPart& prev_layer_part : storage.Layers[layer_nr].parts)
				{
		...
		const bool bridge_settings_enabled = false;
		const Ratio support_threshold = Ratio(50 / 100);// bridge_settings_enabled ? settings.get<Ratio>("bridge_skin_support_threshold") : 0.0_r;
		if (support_threshold > 0)
		{
			...
			if ((supported_regions.area() / (skin_outline.area() + 1)) < support_threshold)
			{
				...
				{
					curaIrfan::PointIrfan p0 = poly[0];
					for (unsigned i = 1; i < poly.size(); ++i)
					{
						curaIrfan::PointIrfan p1 = poly[i];
				...
				if (skin_perimeter_lines_over_air.size())
				{
					...
					{
						curaIrfan::PointIrfan p0 = air_line[0];
						for (unsigned i = 1; i < air_line.size(); ++i)
						{
							const curaIrfan::PointIrfan& p1(air_line[i]);
							double dist2 = curaIrfan::vSize2(curaIrfan::operator-(p0 , p1));
							if (dist2 > max_dist2)
							{
								max_dist2 = dist2;
								line_angle = curaIrfan::angle(curaIrfan::operator-(p0 , p1));
							}
							p0 = p1;
						}
					}
		...
		curaIrfan::PointIrfan center1 = islands[idx1].centerOfMass();
		curaIrfan::PointIrfan center2 = islands[idx2].centerOfMass();

		return curaIrfan::angle(curaIrfan::operator-(center2 , center1));
	}
