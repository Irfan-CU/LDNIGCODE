...
static int findAdjacentEnclosingPoly(const ConstPolygonRef& enclosed_inset, const std::vector<ConstPolygonPointer>& possible_enclosing_polys, const coord_tIrfan max_gap)
{
...
{
	const coord_tIrfan outer_wall_line_width = mesh_config.inset0_config.getLineWidth();
	curaIrfan::PointIrfan p = gcode_layer.getLastPlannedPositionOrStartingPosition();
	...
	if (PolygonUtils::moveInside(part.insets[0], p, outer_wall_line_width * 1.1f) != NO_INDEX)
	{
		...
		{
			gcode_layer.addTravel_simple(layer_nr, p);
...
void InsetOrderOptimizer::processHoleInsets()
{
	...
	const coord_tIrfan wall_line_width_0 = mesh_config.inset0_config.getLineWidth();
	const coord_tIrfan wall_line_width_x = mesh_config.insetX_config.getLineWidth();
	const coord_tIrfan max_gap = std::max(wall_line_width_0, wall_line_width_x) * 1.1f; // if polys are closer than this, they are considered adjacent
	const coord_tIrfan wall_0_wipe_dist = MM2INT(0.2);// mesh.settings.get<coord_tIrfan>("wall_0_wipe_dist");
	const bool retract_before_outer_wall = true;// mesh.settings.get<bool>("travel_retract_before_outer_wall");
	const bool outer_inset_first = false || (layer_nr == 0);
	const unsigned int num_insets = part.insets.size();
	constexpr float flow = 1.0;
	bool infill_before_walls = true;
	if (!outer_inset_first && infill_before_walls)
	{
		...
		for (unsigned inset_level = num_insets - 1; inset_level > 0; --inset_level)
		{
			...
			if (insets_that_do_not_surround_holes.size() > 0 && extruder_nr == -1)
			{
	...
	curaIrfan::PointIrfan start_point = gcode_layer.getLastPlannedPositionOrStartingPosition(); // where we are now
	
	const bool optimize_backwards = true;
	if (optimize_backwards)
	{
		// determine the location of the z-seam and use that as the start point
		PathOrderOptimizer order_optimizer(curaIrfan::PointIrfan(), z_seam_config);
	...
	for (unsigned int outer_poly_order_idx = 0; outer_poly_order_idx < order_optimizer.polyOrder.size(); ++outer_poly_order_idx)
	{
		...
		if (hole_inner_walls.size() > 0 && extruder_nr == -1)
		{
			...
			else
			{
				...
				const curaIrfan::PointIrfan z_seam_location = hole_outer_wall[0][outer_poly_start_idx];
...
void InsetOrderOptimizer::processOuterWallInsets(const bool include_outer, const bool include_inners)
{
	...
	const coord_tIrfan wall_0_wipe_dist = MM2INT(0.2);// mesh.settings.get<coord_tIrfan>("wall_0_wipe_dist");
	const bool retract_before_outer_wall = true;// mesh.settings.get<bool>("travel_retract_before_outer_wall");
	const bool outer_inset_first =false
		|| (layer_nr == 0 && true);
	...
	if (part_inner_walls.size() > 0 && extruder_nr == -1)
	{
		//gcode_writer.setExtruder_addPrime(storage, gcode_layer, extruder_nr);
		gcode_layer.setIsInside(true); // going to print stuff inside print object
		...
		order_optimizer.optimize();
		const unsigned outer_poly_start_idx = gcode_layer.locateFirstSupportedVertex(*inset_polys[0][0], order_optimizer.polyStart[0]);
		const curaIrfan::PointIrfan z_seam_location = (*inset_polys[0][0])[outer_poly_start_idx];
	...
	else if (include_outer && extruder_nr == -1)
	{
