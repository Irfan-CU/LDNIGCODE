...
#include <functional>
	...
	public:
		 ...
		 */
		SparseGrid(coord_tIrfan cell_size, size_t elem_reserve = 0U, float max_load_factor = 1.0f);
		 ...
		 */
		std::vector<Elem> getNearby(const curaIrfan::PointIrfan &query_pt, coord_tIrfan radius) const;
		 ...
		 */
		bool getNearest(const curaIrfan::PointIrfan &query_pt, coord_tIrfan radius, Elem &elem_nearest,
		 ...
		 */
		bool processNearby(const curaIrfan::PointIrfan &query_pt, coord_tIrfan radius,
		 ...
		 * \return Whether we need to continue processing after this function
		 */
		bool processLine(const std::pair<curaIrfan::PointIrfan, curaIrfan::PointIrfan> query_line,
			const std::function<bool(const Elem&)>& process_elem_func) const;

		coord_tIrfan getCellSize() const;

	protected:
		using GridPoint = curaIrfan::PointIrfan;
		using grid_coord_t = coord_tIrfan;
		 ...
		 * \return Whether we need to continue processing after this function
		 */
		bool processLineCells(const std::pair<curaIrfan::PointIrfan, curaIrfan::PointIrfan> line,
		 ...
		 */
		GridPoint toGridPoint(const curaIrfan::PointIrfan& point) const;
		 ...
		 */
		grid_coord_t toGridCoord(const coord_tIrfan& coord) const;
		 ...
		 */
		curaIrfan::PointIrfan toLowerCorner(const GridPoint& location) const;
		 ...
		 */
		coord_tIrfan toLowerCoord(const grid_coord_t& grid_coord) const;

		/*! \brief Map from grid locations (GridPoint) to elements (Elem). */
		GridMap m_grid;
		/*! \brief The cell (square) size. */
		coord_tIrfan m_cell_size;
...
#define SGI_THIS SparseGrid<ElemT>
	...
	SGI_TEMPLATE
		SGI_THIS::SparseGrid(coord_tIrfan cell_size, size_t elem_reserve, float max_load_factor)
	{
		assert(cell_size > 0U);
	...
	SGI_TEMPLATE
		typename SGI_THIS::GridPoint SGI_THIS::toGridPoint(const curaIrfan::PointIrfan &point)  const
	{
		return curaIrfan::PointIrfan(toGridCoord(point.X), toGridCoord(point.Y));
	}

	SGI_TEMPLATE
		typename SGI_THIS::grid_coord_t SGI_THIS::toGridCoord(const coord_tIrfan& coord)  const
	{
	...
	SGI_TEMPLATE
		typename curaIrfan::PointIrfan SGI_THIS::toLowerCorner(const GridPoint& location)  const
	{
		return cura::Point(toLowerCoord(location.X), toLowerCoord(location.Y));
	}

	SGI_TEMPLATE
		typename coord_tIrfan SGI_THIS::toLowerCoord(const grid_coord_t& grid_coord)  const
	{
		// This mapping via truncation results in the cells with
		// GridPoint.x==0 being twice as large and similarly for
		// GridPoint.y==0.  This doesn't cause any incorrect behavior,
	...
	SGI_TEMPLATE
		bool SGI_THIS::processLineCells(
			const std::pair<curaIrfan::PointIrfan, curaIrfan::PointIrfan> line,
			const std::function<bool(GridPoint)>& process_cell_func)
	{
			...
			const std::function<bool(GridPoint)>& process_cell_func) const
	{
		...
		curaIrfan::PointIrfan start = line.first;
		curaIrfan::PointIrfan end = line.second;
		if (end.X < start.X)
		...
		const coord_tIrfan y_diff = end.Y - start.Y;
		...
		{ // for all Y from start to end
			...
			coord_tIrfan nearest_next_y = toLowerCoord(cell_y + ((nonzero_sign(cell_y) == y_dir || cell_y == 0) ? y_dir : coord_tIrfan(0)));
			...
			{
				coord_tIrfan area = (end.X - start.X) * (nearest_next_y - start.Y);
				// corresponding_x: the x coordinate corresponding to nearest_next_y
				coord_tIrfan corresponding_x = start.X + area / y_diff;
	...
	SGI_TEMPLATE
		bool SGI_THIS::processNearby(const curaIrfan::PointIrfan &query_pt, coord_tIrfan radius,
			const std::function<bool(const Elem&)>& process_func) const
	{
		curaIrfan::PointIrfan min_loc(query_pt.X - radius, query_pt.Y - radius);
		curaIrfan::PointIrfan max_loc(query_pt.X + radius, query_pt.Y + radius);

		GridPoint min_grid = toGridPoint(min_loc);
		GridPoint max_grid = toGridPoint(max_loc);

		for (coord_tIrfan grid_y = min_grid.Y; grid_y <= max_grid.Y; ++grid_y)
		{
			for (coord_tIrfan grid_x = min_grid.X; grid_x <= max_grid.X; ++grid_x)
			{
	...
	SGI_TEMPLATE
		bool SGI_THIS::processLine(const std::pair<curaIrfan::PointIrfan, curaIrfan::PointIrfan> query_line,
			const std::function<bool(const Elem&)>& process_elem_func) const
	{
	...
	SGI_TEMPLATE
		...
		SGI_THIS::getNearby(const curaIrfan::PointIrfan &query_pt, coord_tIrfan radius) const
	{
		std::vector<Elem> ret;
	...
	SGI_TEMPLATE
		bool SGI_THIS::getNearest(
			const curaIrfan::PointIrfan &query_pt, coord_tIrfan radius, Elem &elem_nearest,
	...
	SGI_TEMPLATE
		coord_tIrfan SGI_THIS::getCellSize() const
	{
		return m_cell_size;
	}
