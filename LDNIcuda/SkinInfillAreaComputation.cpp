...
#define MIN_AREA_SIZE (0.4 * 0.4) 
	...
	coord_tIrfan SkinInfillAreaComputation::getWallLineWidth0(const SliceDataStorage& storage, const int& layer_nr)
	{
		coord_tIrfan wall_line_width_0 = MM2INT(0.35);// mesh.settings.get<coord_tIrfan>("wall_line_width_0");
		if (layer_nr == 0)
		{
	...
	coord_tIrfan SkinInfillAreaComputation::getWallLineWidthX(const SliceDataStorage& storage, const int& layer_nr)
	{
		coord_tIrfan wall_line_width_x =MM2INT(0.30);// mesh.settings.get<coord_tIrfan>("wall_line_width_x");
		if (layer_nr == 0)
		{
	...
	SkinInfillAreaComputation::SkinInfillAreaComputation(const int& layer_nr, SliceDataStorage& storage, bool process_infill)
		...
		, wall_line_width_x(getWallLineWidthX(storage, layer_nr))
		...
		, top_skin_expand_distance(MM2INT(1))//mesh.settings.get<coord_tIrfan>("top_skin_expand_distance"))
		, bottom_skin_expand_distance(MM2INT(1))//mesh.settings.get<coord_tIrfan>("bottom_skin_expand_distance"))
	...
	{
		Polygons result;
		if (layer2_nr >= static_cast<int>(storage.Layers.size()))
		{
			return result;
		}
		const SliceLayer& layer2 = storage.Layers[layer2_nr];
	...
	int SkinInfillAreaComputation::getReferenceWallIdx(coord_tIrfan& preshrink) const
	{
		for (int wall_idx = wall_line_count; wall_idx > 0; wall_idx--)
		{
			coord_tIrfan wall_line_width = (wall_idx > 1) ? wall_line_width_x : wall_line_width_0;
			int next_wall_idx = wall_idx - 1;
			coord_tIrfan next_wall_line_width = (next_wall_idx > 1) ? wall_line_width_x : (next_wall_idx == 0) ? 0 : wall_line_width_0;
			coord_tIrfan diff_to_next_wall = (wall_line_width + next_wall_line_width) / 2;
	...
	void SkinInfillAreaComputation::generateSkinAndInfillAreas()
	{
		...
		//printf("the layer number inside generateSkinAndInfillAreas is %d and the layers size is %d \n", layer_nr, storage.Layers.size());
		
		//printf("the bottom_layer_count is %d and top_layer_count is %d \n", bottom_layer_count, top_layer_count);
	...
	{
		//printf("the bottom layer count and layer ny is %d and %d \n", layer_nr, bottom_layer_count);
		if (static_cast<int>(layer_nr - bottom_layer_count) >= 0 && bottom_layer_count > 0)
		{
			...
			const double min_infill_area = 0.0;
	...
	{
		//printf("the top layer count and layer nr is%d for skin infill is %d \n", layer_nr, top_layer_count);
		if (static_cast<int>(layer_nr + top_layer_count) < static_cast<int>(storage.Layers.size()) && top_layer_count > 0)
		{
			...
			const double min_infill_area = 0;// mesh.settings.get<double>("min_infill_area");
			if (min_infill_area > 0.0)
			{
				not_air.removeSmallAreas(min_infill_area);
			}
	...
	void SkinInfillAreaComputation::applySkinExpansion(const Polygons& original_outline, Polygons& upskin, Polygons& downskin)
	{
		...
		coord_tIrfan top_outset = top_skin_expand_distance;
		...
		top_min_width_int = std::max(coord_tIrfan(MM2INT(0)), top_min_width_int - top_reference_wall_expansion / 2); // if the min width is smaller than the pre-shrink then areas smaller than min_width will exist
		bottom_min_width = std::max(coord_tIrfan(MM2INT(0)), bottom_min_width - bottom_reference_wall_expansion / 2); // if the min width is smaller than the pre-shrink then areas smaller than min_width will exist
		...
		top_outset += top_min_width_int; // increase the expansion distance to compensate for the min_width shrinkage
		...
		if (top_outset)
		{
			upskin = upskin.offset(-top_min_width_int).offset(top_outset).unionPolygons(upskin).intersection(original_outline);
		}
	...
	{
		const size_t roofing_layer_count =0 ;//esh.settings.get<size_t>("roofing_layer_count");
		const size_t wall_idx = 3;// std::min(size_t(2), mesh.settings.get<size_t>("wall_line_count"));
	...
	/*void SkinInfillAreaComputation::generateInfillSupport(SliceDataStorage& storage)
	{
		const coord_tIrfan layer_height = mesh.settings.get<coord_tIrfan>("layer_height");
	...
	*/
	...
	void SkinInfillAreaComputation::generateGradualInfill(SliceDataStorage& storage)
	{
		...
		{ // loop also over layers which don't contain infill cause of bottom_ and top_layer to initialize their infill_area_per_combine_per_density
			...
			for (SliceLayerPart& part : layer.parts)
			{
				...
				{
					int min_layer = layer_idx + infill_step * gradual_infill_step_layer_count + static_cast<size_t>(layer_skip_count);
					int max_layer = layer_idx + (infill_step + 1) * gradual_infill_step_layer_count;

					for (float upper_layer_idx = min_layer; upper_layer_idx <= max_layer; upper_layer_idx += layer_skip_count)
					{
						if (upper_layer_idx >= storage.Layers.size())
						{
							less_dense_infill.clear();
							break;
						}
						const SliceLayer& upper_layer = storage.Layers[static_cast<size_t>(upper_layer_idx)];
	...
	void SkinInfillAreaComputation::combineInfillLayers(SliceDataStorage& storage)
	{
		...
		for (int layer_idx = min_layer; layer_idx <= max_layer; layer_idx += amount) //Skip every few layers, but extrude more.
		{
			...
			{
				if (layer_idx < static_cast<int>(combine_count_here))
				{
					break;
				}

				int lower_layer_idx = layer_idx - combine_count_here;
				if (lower_layer_idx < min_layer)
				{
					break;
				}
				SliceLayer* lower_layer = &storage.Layers[lower_layer_idx];
				for (SliceLayerPart& part : layer->parts)
				{
					...
					{ // go over each density of gradual infill (these density areas overlap!)
						...
						//printf("\n ********yes inside the combines infill function****** \n");
						Polygons result;
