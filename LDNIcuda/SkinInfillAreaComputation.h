...
#include "Coord_tIrfan.h"
	...
	public:
		 ...
		 */
		SkinInfillAreaComputation(const int& layer_nr, SliceDataStorage& storage, bool process_infill);
		 ...
		 */
	   static void combineInfillLayers(SliceDataStorage& mesh);
		 ...
		 */
		static void generateGradualInfill(SliceDataStorage& storage);
	...
	protected:
		...
		SliceDataStorage& storage;//!< The storage where the layer outline information (input) is stored and where the skin insets and fill areas (output) are stored.
		...
		const coord_tIrfan skin_line_width; //!< The line width of the skin.
		const coord_tIrfan wall_line_width_0; //!< The line width of the outer wall
		const coord_tIrfan wall_line_width_x; //!< The line width of the inner most wall
		const coord_tIrfan innermost_wall_line_width; //!< width of the innermost wall lines
		const coord_tIrfan infill_skin_overlap; //!< overlap distance between infill and skin
		...
		coord_tIrfan top_reference_wall_expansion; //!< The horizontal expansion to apply to the top reference wall in order to shrink the top skin
		coord_tIrfan bottom_reference_wall_expansion; //!< The horizontal expansion to apply to the bottom reference wall in order to shrink the bottom skin
		coord_tIrfan top_skin_expand_distance; //!< The distance by which the top skins should be larger than the original top skins.
		coord_tIrfan bottom_skin_expand_distance; //!< The distance by which the bottom skins should be larger than the original bottom skins.
	...
	private:
		static coord_tIrfan getSkinLineWidth(const SliceDataStorage& storage, const int& layer_nr); //!< Compute the skin line width, which might be different for the first layer.
		static coord_tIrfan getWallLineWidth0(const SliceDataStorage& storage, const int& layer_nr); //!< Compute the outer wall line width, which might be different for the first layer
		static coord_tIrfan getWallLineWidthX(const SliceDataStorage& storage, const int& layer_nr); //!< Compute the inner wall line widths, which might be different for the first layer
		static coord_tIrfan getInfillSkinOverlap(const SliceDataStorage& storage, const int& layer_nr, const coord_tIrfan& innermost_wall_line_width); //!< Compute the infill_skin_overlap
		 ...
		 */
		int getReferenceWallIdx(coord_tIrfan& preshrink) const;
