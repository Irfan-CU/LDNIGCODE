...
#include <limits> //To find the maximum for coord_tIrfan.

#include "LayerIndex.h"//To store the layer on which we comb.
#include "Optional.h"
#include "Polygon.h"
...
namespace cura
{
	...
	private:
		...
		public:
			...
			curaIrfan::PointIrfan in_or_mid; //!< The point on the inside boundary, or in between the inside and outside boundary if the start/end point isn't inside the inside boudary
			curaIrfan::PointIrfan out; //!< The point on the outside boundary
			 ...
			 */
			//Crossing(const curaIrfan::PointIrfan& dest_point, const bool dest_is_inside, const unsigned int dest_part_idx, const unsigned int dest_part_boundary_crossing_poly_idx, const Polygons& boundary_inside, const LocToLineGrid* inside_loc_to_line);
			 ...
			 */
			//void findCrossingInOrMid(const PartsView& partsView_inside, const curaIrfan::PointIrfan close_to);
			 ...
			 */
			//bool findOutside(const Polygons& outside, const curaIrfan::PointIrfan close_to, const bool fail_on_unavoidable_obstacles, Comb& comber);

		private:
			const curaIrfan::PointIrfan dest_point; //!< Either the eventual startPoint or the eventual endPoint of this combing move
			 ...
			 */
			std::shared_ptr<std::pair<ClosestPolygonPoint, ClosestPolygonPoint>> findBestCrossing(const Polygons& outside, ConstPolygonRef from, curaIrfan::PointIrfan estimated_start, curaIrfan::PointIrfan estimated_end, Comb& comber);
		};
		...
		const coord_tIrfan offset_from_outlines; //!< Offset from the boundary of a part to the comb path. (nozzle width / 2)
		const coord_tIrfan max_moveInside_distance2; //!< Maximal distance of a point to the Comb::boundary_inside which is still to be considered inside. (very sharp corners not allowed :S)
		const coord_tIrfan offset_from_inside_to_outside; //!< The sum of the offsets for the inside and outside boundary Comb::offset_from_outlines and Comb::offset_from_outlines_outside
		const coord_tIrfan max_crossing_dist2; //!< The maximal distance by which to cross the in_between area between inside and outside
		static const coord_tIrfan max_moveOutside_distance2 = std::numeric_limits<coord_tIrfan>::max(); //!< Any point which is not inside should be considered outside.
		static constexpr coord_tIrfan offset_dist_to_get_from_on_the_polygon_to_outside = 40; //!< in order to prevent on-boundary vs crossing boundary confusions (precision thing)
		static constexpr coord_tIrfan offset_extra_start_end = 100; //!< Distance to move start point and end point toward eachother to extra avoid collision with the boundaries.
	...
	//	LazyInitialization<LocToLineGrid, Comb*, const coord_tIrfan> outside_loc_to_line; //!< The SparsePointGridInclusive mapping locations to line segments of the outside boundary.
		coord_tIrfan move_inside_distance; //!< When using comb_boundary_inside_minimum for combing it tries to move points inside by this amount after calculating the path to move it from the border a bit.
		 ...
		 */
		bool moveInside(Polygons& boundary_inside, bool is_inside, LocToLineGrid* inside_loc_to_line, curaIrfan::PointIrfan& dest_point, unsigned int& start_inside_poly);
	...
	public:
		 ...
		 */
		Comb(const SliceDataStorage& storage, const LayerIndex layer_nr, const Polygons& comb_boundary_inside_minimum, const Polygons& comb_boundary_inside_optimal, coord_tIrfan offset_from_outlines, coord_tIrfan travel_avoid_distance, coord_tIrfan move_inside_distance);
		 ...
		 */
		bool calc(const ExtruderTrain& train, curaIrfan::PointIrfan startPoint, curaIrfan::PointIrfan endPoint, CombPaths& combPaths, bool startInside, bool endInside, coord_tIrfan max_comb_distance_ignored);
	};
