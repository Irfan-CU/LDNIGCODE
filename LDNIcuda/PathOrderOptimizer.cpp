...
#include "LinesPolygonCrossing.h"
#include "CombPathh.h"
...
void PathOrderOptimizer::optimize()
{
	...
	for (unsigned poly_idx = 0; poly_idx < polygons.size(); ++poly_idx) /// find closest point to initial starting point within each polygon +initialize picked
	{
		...
		case EZSeamType::RANDOM:
			...
			printf("inside  PathOrderOptimizer::optimize() at line 21\n");
			break;
		default:
	...
	curaIrfan::PointIrfan prev_point;
	switch (config.type)
	{
	case EZSeamType::USER_SPECIFIED:
	...
	for (unsigned int poly_order_idx = 0; poly_order_idx < polygons.size(); poly_order_idx++) /// actual path order optimizer
	{
		...
		for (unsigned int poly_idx = 0; poly_idx < polygons.size(); poly_idx++)
		{
			...
			const curaIrfan::PointIrfan& p = (*polygons[poly_idx])[polyStart[poly_idx]];
			curaIrfan::PointIrfan check = curaIrfan::operator-(p, prev_point);
			float dist2 = curaIrfan::vSize2f(check);
			/*
		...
		{
			printf("Failed to find next closest polygon.\n");
		}
	}
	/*
...
int PathOrderOptimizer::getClosestPointInPolygon(curaIrfan::PointIrfan prev_point, int poly_idx)
{
	...
	for (unsigned int point_idx = 0; point_idx < poly.size(); point_idx++)
	{
		...
		const curaIrfan::PointIrfan& p2 = poly[(point_idx + 1) % poly.size()];
		// when type is SHARPEST_CORNER, actual distance is ignored, we use a fixed distance and decision is based on curvature only
		curaIrfan::PointIrfan check = curaIrfan::operator-(p1, prev_point);
		float dist_score = (config.type == EZSeamType::SHARPEST_CORNER && config.corner_pref != EZSeamCornerPrefType::Z_SEAM_CORNER_PREF_NONE) ? 10000 : curaIrfan::vSize2(check);
		const float corner_angle = 20;// LinearAlg2D::getAngleLeft(p0, p1, p2) / 3.14; // 0 -> 2
		float corner_shift;
		...
		case EZSeamCornerPrefType::Z_SEAM_CORNER_PREF_WEIGHTED:
			...
			break;														  		case EZSeamCornerPrefType::Z_SEAM_CORNER_PREF_NONE:
		default:
			// do nothing
			break;
		}
...
static inline bool pointsAreCoincident(const curaIrfan::PointIrfan& a, const curaIrfan::PointIrfan& b)
{
	return curaIrfan::vSize2(curaIrfan::operator-(a, b)) < 25; // points are closer than 5uM, consider them coincident
}
...
void LineOrderOptimizer::optimize(bool find_chains)
{
	...
	if (find_chains)
	{
		...
		for (unsigned int poly_idx = 0; poly_idx < polygons.size(); poly_idx++)
		{
			...
			for (unsigned point_idx = 0; point_idx < 2; ++point_idx)
			{
				...
				const curaIrfan::PointIrfan& p = (*polygons[poly_idx])[point_idx];
	...
	curaIrfan::PointIrfan prev_point = startPoint;
	...
	for (unsigned int order_idx = 0; order_idx < polygons.size(); order_idx++) /// actual path order optimizer
	{
		...
		if (best_line_idx > -1) /// should always be true; we should have been able to identify the best next polygon
		{
			...
			const curaIrfan::PointIrfan& line_end = best_line[line_end_point_idx];
			prev_point = line_end;
		...
		{
			printf("Failed to find next closest line.\n");
		}
	}
	if (loc_to_line != nullptr)
	{
...
float LineOrderOptimizer::combingDistance2(const curaIrfan::PointIrfan &p0, const curaIrfan::PointIrfan &p1)
{
	if (loc_to_line == nullptr)
	{
	...
	{
		float dist = 0;
		curaIrfan::PointIrfan last_point = p0;
		for (const curaIrfan::PointIrfan& comb_point : comb_path)
		{
			dist += curaIrfan::vSize(curaIrfan::operator-(comb_point, last_point));
			last_point = comb_point;
		}
		return dist * dist;
	}
	...
	return curaIrfan::vSize2f(curaIrfan::operator-(p1,p0)) * 10000;
}

inline void LineOrderOptimizer::updateBestLine(unsigned int poly_idx, int& best, float& best_score, curaIrfan::PointIrfan prev_point, int just_point)
{
	...
	const curaIrfan::PointIrfan& p0 = (*polygons[poly_idx])[0];
	const curaIrfan::PointIrfan& p1 = (*polygons[poly_idx])[1];

	if (just_point != 1)
	{ /// check distance to first point on line (0)
		float score = curaIrfan::vSize2f(curaIrfan::operator-(p0, prev_point));
		if (score < best_score
	...
	{ /// check distance to second point on line (1)
		float score = curaIrfan::vSize2f(curaIrfan::operator-(p1, prev_point));
		if (score < best_score
			&& combing_boundary != nullptr
