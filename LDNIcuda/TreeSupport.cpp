...
#include "LayerIndex.h"
 //Creating the correct branch angles.
#include "IntpointIrfan.h"
#include "logoutput.h"
#include "math.h" //For round_up_divide and PI.
...
#define PROGRESS_WEIGHT_AREAS 1 //Creating support areas.
	...
	TreeSupport::TreeSupport(const SliceDataStorage& storage)
	{
		...
		const coord_tIrfan xy_distance = MM2INT(0.875);// mesh_group_settings.get<coord_tIrfan>("support_xy_distance");
		...
		const coord_tIrfan maximum_move_distance
			= (angle < (2*M_PI) / 4) ? (coord_tIrfan)(tan(angle) * layer_height) : std::numeric_limits<coord_tIrfan>::max();
	...
	{
		const bool global_use_tree_support = true;// Application::getInstance().current_slice->scene.current_mesh_group->settings.get<bool>("support_tree_enable");
	...
	void TreeSupport::drawCircles(SliceDataStorage& storage, const std::vector<std::unordered_set<Node*>>& contact_nodes)
	{
		...
		{
			const double angle = static_cast<double>(i) / CIRCLE_RESOLUTION * (3.14*M_PI);
		...
		const coord_tIrfan line_width = MM2INT(0.35);// mesh_group_settings.get<coord_tIrfan>("support_line_width");
		const coord_tIrfan resolution = MM2INT(0.175);// mesh_group_settings.get<coord_tIrfan>("support_tree_collision_resolution");
...
#pragma omp parallel for shared(storage, contact_nodes)
	// Use a signed type for the loop counter so MSVC compiles (because it uses OpenMP 2.0, an old version).
		for (int layer_nr = 0; layer_nr < static_cast<int>(contact_nodes.size()); layer_nr++)
		{
			...
			//Draw the support areas and add the roofs appropriately to the support roof instead of curaIrfan::normal areas.
			for (const Node* p_node : contact_nodes[layer_nr])
			{
				...
				for (curaIrfan::PointIrfan corner : branch_circle)
				{
					if (node.distance_to_top < tip_layers) //We're in the tip.
					{
						const int mul = node.skin_direction ? 1 : -1;
						corner = curaIrfan::PointIrfan(corner.X * (0.5 + scale / 2) + mul * corner.Y * (0.5 - scale / 2),
							mul * corner.X * (0.5 - scale / 2) + corner.Y * (0.5 + scale / 2));
					}
					else
					{
						corner = curaIrfan::operator*(corner,(1 + static_cast<double>(node.distance_to_top - tip_layers) * diameter_angle_scale_factor));
					}
					circle.add(curaIrfan::operator+(node.position, corner));
				}
			...
			if (support_bottom_enable)
			{
				...
				const coord_tIrfan support_interface_resolution = MM2INT(0.3);// mesh_group_settings.get<coord_tIrfan>("support_interface_skip_height");
				const size_t support_interface_skip_layers = round_up_divide(support_interface_resolution, layer_height);
				const coord_tIrfan support_bottom_height = MM2INT(1.0);// mesh_group_settings.get<coord_tIrfan>("support_bottom_height");
				...
				support_layer = support_layer.difference(floor_layer.offset(10)); //Subtract the support floor from the curaIrfan::normal support.
			}
...
#pragma omp critical (progress)
	...
	void TreeSupport::dropNodes(std::vector<std::unordered_set<Node*>>& contact_nodes)
	{
		...
		for (size_t layer_nr = contact_nodes.size() - 1; layer_nr > 0; layer_nr--) //Skip layer 0, since we can't drop down the vertices there.
		{
			...
			std::vector<std::unordered_map<curaIrfan::PointIrfan, Node*>> nodes_per_part;
			...
			for (Node* p_node : layer_contact_nodes)
			{
				 ...
				 */
				coord_tIrfan closest_part_distance2 = std::numeric_limits<coord_tIrfan>::max();
				...
				for (size_t part_index = 0; part_index < parts.size(); part_index++)
				{
					...
					const coord_tIrfan distance2 = curaIrfan::vSize2(curaIrfan::operator-(node.position, closest_point.location));
			...
			for (const std::unordered_map<curaIrfan::PointIrfan, Node*>& group : nodes_per_part)
			{
				std::unordered_set<curaIrfan::PointIrfan> points_to_buildplate;
				for (const std::pair<curaIrfan::PointIrfan, Node*>& entry : group)
				{
			...
			for (size_t group_index = 0; group_index < nodes_per_part.size(); group_index++)
			{
				...
				for (const std::pair<curaIrfan::PointIrfan, Node*>& entry : nodes_per_part[group_index])
				{
					...
					const std::vector<curaIrfan::PointIrfan>& neighbours = mst.adjacentNodes(node.position);
					if (neighbours.size() == 1 && curaIrfan::vSize2(curaIrfan::operator-(neighbours[0], node.position)) < maximum_move_distance * maximum_move_distance && mst.adjacentNodes(neighbours[0]).size() == 1) //We have just two nodes left, and they're very close!
					{
						//Insert a completely new node and let both original nodes fade.
						curaIrfan::PointIrfan next_position = curaIrfan::operator/((curaIrfan::operator+(node.position,neighbours[0])), 2); //Average position of the two nodes.

						const coord_tIrfan branch_radius_node = [&]() -> coord_tIrfan
						{
						...
						{
							//Avoid collisions.
							const coord_tIrfan maximum_move_between_samples = maximum_move_distance + radius_sample_resolution + 100; //100 micron extra for rounding errors.
						...
						else
						{
							...
							const coord_tIrfan distance = curaIrfan::vSize(curaIrfan::operator-(node.position, closest_point_on_border.location));
							//Try moving a bit further inside: Current distance + 1 step.
							curaIrfan::PointIrfan moved_inside = next_position;
							...
							curaIrfan::PointIrfan difference = curaIrfan::operator-(moved_inside, node.position);
							if (curaIrfan::vSize2(difference) > maximum_move_distance * maximum_move_distance)
							{
								difference = curaIrfan::normal(difference, maximum_move_distance);
							}
							next_position = curaIrfan::operator+(node.position, difference);
						}
					...
					else if (neighbours.size() > 1) //Don't merge leaf nodes because we would then incur movement greater than the maximum move distance.
					{
						...
						for (const curaIrfan::PointIrfan& neighbour : neighbours)
						{
							if (curaIrfan::vSize2(curaIrfan::operator-(neighbour, node.position)) < maximum_move_distance * maximum_move_distance)
							{
				...
				for (const std::pair<curaIrfan::PointIrfan, Node*>& entry : nodes_per_part[group_index])
				{
					...
					if (group_index > 0 && volumes_.getCollision(0, layer_nr).inside(node.position))
					{
						...
						if (curaIrfan::vSize2(curaIrfan::operator-(node.position, to_outside.location)) >= branch_radius_node * branch_radius_node) //Too far inside.
						{
					...
					curaIrfan::PointIrfan next_layer_vertex = node.position;
					const std::vector<curaIrfan::PointIrfan> neighbours = mst.adjacentNodes(node.position);
					if (neighbours.size() > 1 || (neighbours.size() == 1 && curaIrfan::vSize2(curaIrfan::operator-(neighbours[0],node.position)) >= maximum_move_distance * maximum_move_distance)) //Only nodes that aren't about to collapse.
					{
						//Move towards the average position of all neighbours.
						curaIrfan::PointIrfan sum_direction(0, 0);
						for (const curaIrfan::PointIrfan& neighbour : neighbours)
						{
							curaIrfan::operator+=(sum_direction, curaIrfan::operator-(neighbour, node.position));
							
						}
						if (curaIrfan::vSize2(sum_direction) <= maximum_move_distance * maximum_move_distance)
						{
							curaIrfan::operator+=(next_layer_vertex, sum_direction);
						}
						else
						{
							curaIrfan::operator+=(next_layer_vertex, curaIrfan::normal(sum_direction, maximum_move_distance));
							//next_layer_vertex += curaIrfan::normal(sum_direction, maximum_move_distance);
						}
					}
					...
					else
					{
						...
						const coord_tIrfan distance = curaIrfan::vSize(curaIrfan::operator-(node.position, closest_point_on_border.location));
						//Try moving a bit further inside: Current distance + 1 step.
						curaIrfan::PointIrfan moved_inside = next_layer_vertex;
						PolygonUtils::ensureInsideOrOutside(volumes_.getInternalModel(branch_radius_node, layer_nr - 1), moved_inside, closest_point_on_border, distance + maximum_move_distance);
						curaIrfan::PointIrfan difference = curaIrfan::operator-(moved_inside, node.position);
						if (curaIrfan::vSize2(difference) > maximum_move_distance * maximum_move_distance)
						{
						...
						next_layer_vertex =curaIrfan::operator+(node.position, difference);
					}
	...
	{
		const coord_tIrfan point_spread = MM2INT(1);// mesh.settings.get<coord_tIrfan>("support_tree_branch_distance"); ;//
		...
		const curaIrfan::PointIrfan bounding_box_size = curaIrfan::operator-(bounding_box.max, bounding_box.min);
		...
		for (auto x = -rotated_dims.X; x <= rotated_dims.X; x += point_spread)
		{
			for (auto y = -rotated_dims.Y; y <= rotated_dims.Y; y += point_spread)
			{
				...
				const auto pt = curaIrfan::operator+(curaIrfan::rotate(curaIrfan::PointIrfan(x, y), rotate_angle), centre);
		...
		const size_t support_roof_layers = [&]() -> size_t
		{
			...
			{
				return round_divide(MM2INT(1), MM2INT(0.01 * 15)); //How many roof layers, if roof is enabled.
			}
			else
			{
				return 0;
		...
		const coord_tIrfan half_overhang_distance = tan(1.0472) * layer_height / 2;
		for (size_t layer_nr = 1; static_cast<int>(layer_nr) < static_cast<int>(mesh.overhang_areas.size()) - static_cast<int>(z_distance_top_layers); layer_nr++)
		{
			...
			for (const ConstPolygonRef overhang_part : overhang)
			{
				...
				for (curaIrfan::PointIrfan candidate : grid_points)
				{
					if (overhang_bounds.contains(candidate))
					{
						constexpr coord_tIrfan distance_inside = 0; //Move point towards the border of the polygon if it is closer than half the overhang distance: Catch points that fall between overhang areas on constant surfaces.
				...
				{
					curaIrfan::PointIrfan candidate = bounding_box.getMiddle();
	...
	ModelVolumes::ModelVolumes(const SliceDataStorage& storage, coord_tIrfan xy_distance, coord_tIrfan max_move,
		coord_tIrfan radius_sample_resolution) :
	...
	const Polygons& ModelVolumes::getCollision(coord_tIrfan radius, LayerIndex layer_idx) const
	{
	...
	const Polygons& ModelVolumes::getAvoidance(coord_tIrfan radius, LayerIndex layer_idx) const
	{
		radius = ceilRadius(radius);
		RadiusLayerPair key{ radius, layer_idx };
	...
	const Polygons& ModelVolumes::getInternalModel(coord_tIrfan radius, LayerIndex layer_idx) const
	{
		radius = ceilRadius(radius);
		RadiusLayerPair key{ radius, layer_idx };
	...
	coord_tIrfan ModelVolumes::ceilRadius(coord_tIrfan radius) const
	{
