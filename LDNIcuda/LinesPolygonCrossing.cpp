...
LinePolygonsCrossings::Crossing::Crossing(const coord_tIrfan x, const size_t point_idx)
		: x(x)
		, point_idx(point_idx)
	{
	}
	...
	LinePolygonsCrossings::PolyCrossings::PolyCrossings(const size_t poly_idx)
		: poly_idx(poly_idx)
		, min(std::numeric_limits<coord_tIrfan>::max(), NO_INDEX), max(std::numeric_limits<coord_tIrfan>::min(), NO_INDEX)
		, n_crossings(0)
	{
	}
	...
	bool LinePolygonsCrossings::calcScanlineCrossings(bool fail_on_unavoidable_obstacles)
	{
		...
		for (unsigned int poly_idx = 0; poly_idx < boundary.size(); poly_idx++)
		{
			...
			curaIrfan::PointIrfan p0 = transformation_matrix.apply(poly[poly.size() - 1]);
			for (unsigned int point_idx = 0; point_idx < poly.size(); point_idx++)
			{
				curaIrfan::PointIrfan p1 = transformation_matrix.apply(poly[point_idx]);
				...
				{ // if line segment crosses the line through the transformed start and end point (aka scanline)
					...
					const coord_tIrfan x = p0.X + (p1.X - p0.X) * (transformed_startPoint.Y - p0.Y) / (p1.Y - p0.Y); // intersection point between line segment and the scanline
	...
	bool LinePolygonsCrossings::lineSegmentCollidesWithBoundary()
	{
		...
		transformation_matrix = curaIrfan::PointMatrix(diff);
	...
	{
		if (curaIrfan::shorterThen(curaIrfan::operator-(endPoint, startPoint), max_comb_distance_ignored) || !lineSegmentCollidesWithBoundary())
		{
	...
	void LinePolygonsCrossings::generateBasicCombingPath(PolyCrossings& polyCrossings, CombPath& combPath)
	{
		...
		combPath.push_back(transformation_matrix.unapply(curaIrfan::PointIrfan(polyCrossings.min.x - std::abs(dist_to_move_boundary_point_outside), transformed_startPoint.Y)));

		// follow the path in the same direction as the winding order of the boundary polygon
		std::vector<curaIrfan::PointIrfan> fwd_points;
		curaIrfan::PointIrfan prev = combPath.back();
		coord_tIrfan fwd_len = 0;
		...
		{
			const curaIrfan::PointIrfan p = PolygonUtils::getBoundaryPointWithOffset(poly, point_idx, dist_to_move_boundary_point_outside);
			fwd_points.push_back(p);
			fwd_len += curaIrfan::vSize(curaIrfan::operator-(p,prev));
			prev = p;
		}

		const curaIrfan::PointIrfan last = transformation_matrix.unapply(curaIrfan::PointIrfan(polyCrossings.max.x + std::abs(dist_to_move_boundary_point_outside), transformed_startPoint.Y));

		if (fwd_points.size() > 0)
		{
			fwd_len += curaIrfan::vSize(curaIrfan::operator-(last, fwd_points.back()));
		}

		// follow the path in the opposite direction of the winding order of the boundary polygon
		std::vector<curaIrfan::PointIrfan> rev_points;
		prev = combPath.back();
		coord_tIrfan rev_len = 0;
			...
			; point_idx = (point_idx > 0) ? (point_idx - 1) : (poly.size() - 1))
		{
			...
			rev_len += curaIrfan::vSize(curaIrfan::operator-(p, prev));
			prev = p;
			if (rev_len > fwd_len)
			{
		...
		{
			rev_len += curaIrfan::vSize(curaIrfan::operator-(last, rev_points.back()));
		}
	...
	bool LinePolygonsCrossings::optimizePath(CombPath& comb_path, CombPath& optimized_comb_path)
	{
		...
		for (unsigned int point_idx = 1; point_idx < comb_path.size(); point_idx++)
		{
			...
			curaIrfan::PointIrfan& current_point = optimized_comb_path.back();
			if (PolygonUtils::polygonCollidesWithLineSegment(current_point, comb_path[point_idx], loc_to_line_grid))
			{
				...
				if (point_idx == 1)
				{
					...
					curaIrfan::PointIrfan p = optimized_comb_path.back();
					for (float frac : { 0.9, 0.9, 0.7, 0.5 })
					{
						// slide p towards the second point in the comb path
						p = curaIrfan::operator+(comb_path[1], curaIrfan::operator*((curaIrfan::operator-(p,comb_path[1])), frac));
		...
		if (optimized_comb_path.size() > 1)
		{
			...
			if (!PolygonUtils::polygonCollidesWithLineSegment(optimized_comb_path[n - 2], comb_path.back(), loc_to_line_grid))
			{
				...
				for (float frac : { 0.9, 0.9, 0.7, 0.5 })
				{
					...
					curaIrfan::PointIrfan p = curaIrfan::operator+(optimized_comb_path[n - 2], curaIrfan::operator*((curaIrfan::operator-(optimized_comb_path[n - 1], optimized_comb_path[n - 2])) , frac));
