...
public:
	 ...
	 */
	//Polygons& getInnermostWalls(const size_t max_inset, const SliceMeshStorage& mesh) const;   //	   SliceMeshStorage replaced by SliceDataStorage
...
public:
	...
	//!< a list of angle values which is cycled through to determine the infill angle of each layer
	std::vector<double> roofing_angles; //!< a list of angle values which is cycled through to determine the roofing angle of each layer
	std::vector<double> skin_angles; //!< a list of angle values which is cycled through to determine the skin angle of each layer
...
public:
	...
	std::vector<double> infill_angles;
	std::vector<double> skin_angles;
	std::vector<double> roofing_angles;
	Point3 model_size;
	Point3 model_min;
	Point3 model_max;
	...
	curaIrfan::PointIrfan getZSeamHint() const;
	 ...
	 */
	Polygons getLayerOutlines(const int layer_nr, const bool include_support, const bool include_prime_tower, const bool external_polys_only = false) const;

	void setlayer_thickness(coord_tIrfan layer_thickness1);

	coord_tIrfan getlayer_thickness();
