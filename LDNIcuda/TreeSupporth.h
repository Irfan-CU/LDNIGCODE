...
class LayerIndex;
	...
	public:
		 ...
		 */
		ModelVolumes(const SliceDataStorage& storage, coord_tIrfan xy_distance, coord_tIrfan max_move,
		 ...
		 */
		const Polygons& getCollision(coord_tIrfan radius, LayerIndex layer_idx) const;
		 ...
		 * \return Polygons object
		 */
		const Polygons& getAvoidance(coord_tIrfan radius, LayerIndex layer_idx) const;
		 ...
		 * \return Polygons object
		 */
		const Polygons& getInternalModel(coord_tIrfan radius, LayerIndex layer_idx) const;
	...
	private:
		 ...
		 * \param radius The radius of the node of interest
		 */
		coord_tIrfan ceilRadius(coord_tIrfan radius) const;
		 ...
		 */
		coord_tIrfan radius_sample_resolution_;
	...
	public:
		...
		struct Node
		{
			...
			Node()
				: distance_to_top(0)
				, position(curaIrfan::PointIrfan(0, 0))
			...
			Node(const curaIrfan::PointIrfan position, const size_t distance_to_top, const bool skin_direction, const int support_roof_layers_below, const bool to_buildplate, Node* const parent)
