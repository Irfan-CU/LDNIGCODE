...
#include "Preheat.h"
#include "Flow_Temprature graph.h"
...
Duration Preheat::getTimeToGoFromTempToTemp(const size_t extruder, const Temperature& temp_before, const Temperature& temp_after, const bool during_printing)
	{
		...
		{
			Temperature heat_up_speed = 1.4;// extruder_settings.get<Temperature>("machine_nozzle_heat_up_speed");
			if (during_printing)
			{
				heat_up_speed -= 0.7;// extruder_settings.get<Temperature>("material_extrusion_cool_down_speed");
			}
			time = (temp_after - temp_before) / heat_up_speed;
		}
		else
		{
			Temperature cool_down_speed = 0.8;// extruder_settings.get<Temperature>("machine_nozzle_cool_down_speed");
			if (during_printing)
			{
				cool_down_speed += 0.7;// extruder_settings.get<Temperature>("material_extrusion_cool_down_speed");
			}
	...
	Temperature Preheat::getTemp(const size_t extruder, const double& flow, const bool is_initial_layer)
	{
		//const Settings& extruder_settings = Application::getInstance().current_slice->scene.extruders[extruder].settings;
		Temperature material_print_temperature_layer_0 = 205.0;
		

		if (is_initial_layer && material_print_temperature_layer_0 != 0)
		{
			return material_print_temperature_layer_0;
		}
		//FlowTempGraph material_flow_temp_graph = [[3.5, 200], [7.0, 240]];
		Temperature material_print_temperature = 200;
		bool material_flow_dependent_temperature = false;

		return (material_print_temperature, material_flow_dependent_temperature);
	}
	...
	Preheat::WarmUpResult Preheat::getWarmUpPointAfterCoolDown(double time_window, unsigned int extruder, double temp_start, double temp_mid, double temp_end, bool during_printing)
	{
		...
		{
			printf("warning getWarmUpPointAfterCoolDown returns result outside of the time window!");
		}
		return result;
	}
	...
	Preheat::CoolDownResult Preheat::getCoolDownPointAfterWarmUp(double time_window, unsigned int extruder, double temp_start, double temp_mid, double temp_end, bool during_printing)
	{
		...
		{
			printf("warning getCoolDownPointAfterWarmUp returns result outside of the time window!");
		}
		return result;
	}
