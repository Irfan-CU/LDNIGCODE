...
class LayerPlanBuffer
{
	...
	static constexpr double extra_preheat_time = 1.0; //!< Time to start heating earlier than computed to avoid accummulative discrepancy between actual heating times and computed ones.
...
public:
	LayerPlanBuffer(GCodeExport& gcode)
		: gcode(gcode)
		, extruder_used_in_meshgroup(2, false)
	{ }
...
private:
	 ...
	 */
	LayerPlan* processBuffer(int layernum, coord_tIrfan layer_thickness);
	 ...
	 */
	void addConnectingTravelMove(coord_tIrfan layer_thicknessint,int  layernum, LayerPlan* prev_layer, const LayerPlan* newest_layer);
	 ...
	 */
	//void insertPreheatCommand(ExtruderPlan& extruder_plan_before, const double time_before_extruder_plan_end, const size_t extruder_nr, const double temp);
...
#endif // LAYER_PLAN_BUFFER_H#pragma once
