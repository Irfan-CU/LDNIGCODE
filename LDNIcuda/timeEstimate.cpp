...
#define MINIMUM_PLANNER_SPEED 0.05 // mm/sec
	...
	void TimeEstimateCalculator::setFirmwareDefaults()
	{
		max_feedrate[X_AXIS] = 300;// settings.get<Velocity>("300");
		max_feedrate[Y_AXIS] = 300;// settings.get<Velocity>("machine_max_feedrate_y");
		max_feedrate[Z_AXIS] = 40;// settings.get<Velocity>("machine_max_feedrate_z");
		max_feedrate[E_AXIS] = 45;// settings.get<Velocity>("machine_max_feedrate_e");
		max_acceleration[X_AXIS] = 9000;// settings.get<Acceleration>("machine_max_acceleration_x");
		max_acceleration[Y_AXIS] = 9000;// settings.get<Acceleration>("machine_max_acceleration_y");
		max_acceleration[Z_AXIS] = 100;// settings.get<Acceleration>("machine_max_acceleration_z");
		max_acceleration[E_AXIS] = 1000;// settings.get<Acceleration>("machine_max_acceleration_e");
		max_xy_jerk = 20.0;// settings.get<Velocity>("machine_max_jerk_xy");
		max_z_jerk = 0.4;// settings.get<Velocity>("machine_max_jerk_z");
		max_e_jerk = 5.0;//;// settings.get<Velocity>("machine_max_jerk_e");
		minimumfeedrate = 0.0;// settings.get<Velocity>("machine_minimum_feedrate");
		acceleration = 3000;// settings.get<Acceleration>("machine_acceleration");
	}
	...
	void TimeEstimateCalculator::setAcceleration(const double& acc)
	{
		acceleration = acc;
	}

	void TimeEstimateCalculator::setMaxXyJerk(const double& jerk)
	{
		max_xy_jerk = jerk;
	}
	...
	static inline double max_allowable_speed(const double& acceleration, const double& target_velocity, double distance)
	{
	...
	static inline float estimate_acceleration_distance(const double& initial_rate, const double& target_rate, const double& acceleration)
	{
		if (acceleration == 0)
		{
			return 0.0;
	...
	static inline double intersection_distance(const double& initial_rate, const double& final_rate, const double& acceleration, double distance)
	{
		if (acceleration == 0.0)
		{
			return 0.0;
	...
	static inline double acceleration_time_from_distance(const double& initial_feedrate, const double& distance, const double& acceleration)
	{
	...
	void TimeEstimateCalculator::plan(Position newPos, double feedrate, PrintFeatureType feature)
	{
		...
		for (size_t n = 0; n < NUM_AXIS; n++)
		{
			...
			{
				feedrate_factor = std::min(feedrate_factor, double(max_feedrate[n] / current_abs_feedrate[n]));
			}
		}
		...
		const double safe_speed = vmax_junction;
		...
		if ((blocks.size() > 0) && (previous_nominal_feedrate > 0.0001))
		{
			...
			{
				vmax_junction_factor = double(max_xy_jerk / xy_jerk);
			}
			if (fabs(current_feedrate[Z_AXIS] - previous_feedrate[Z_AXIS]) > max_z_jerk)
			{
				vmax_junction_factor = std::min(vmax_junction_factor, double(max_z_jerk / fabs(current_feedrate[Z_AXIS] - previous_feedrate[Z_AXIS])));
			}
			if (fabs(current_feedrate[E_AXIS] - previous_feedrate[E_AXIS]) > max_e_jerk)
			{
				vmax_junction_factor = std::min(vmax_junction_factor, double(max_e_jerk / fabs(current_feedrate[E_AXIS] - previous_feedrate[E_AXIS])));
			}
		...
		const double v_allowable = max_allowable_speed(-block.acceleration, MINIMUM_PLANNER_SPEED, block.distance);
		...
		calculate_trapezoid_for_block(&block, double(block.entry_speed / block.nominal_feedrate), double(safe_speed / block.nominal_feedrate));
	...
	void TimeEstimateCalculator::recalculate_trapezoids()
	{
		...
		for (unsigned int n = 0; n < blocks.size(); n++)
		{
			...
			if (current)
			{
				...
				if (current->recalculate_flag || next->recalculate_flag)
				{
					...
					calculate_trapezoid_for_block(current, double(current->entry_speed / current->nominal_feedrate), double(next->entry_speed / current->nominal_feedrate));
		...
		if (next != nullptr)
		{
			calculate_trapezoid_for_block(next, double(next->entry_speed / next->nominal_feedrate), double(MINIMUM_PLANNER_SPEED / next->nominal_feedrate));
