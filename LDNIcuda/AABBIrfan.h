...
namespace curaIrfan
{
	...
	public:
		...
		AABB(const PointIrfan& min, const PointIrfan& max); //!< initializes with given min and max
		 ...
		 * ``false`` otherwise.
		 */
		bool contains(const PointIrfan& point) const;

		/*!
		 * Get the middle of the bounding box
		 */
		PointIrfan getMiddle() const;
