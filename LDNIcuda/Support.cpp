...
#include "math.h"
	...
	bool AreaSupport::handleSupportModifierMesh(SliceDataStorage& storage, const Slicer* slicer)
	{
		bool anti_overhang_mesh = false;
		bool support_mesh = false;
		bool support_mesh_drop_down = true;
		if (!anti_overhang_mesh && !support_mesh)
		{
			return false;
		}
		//enum ModifierType { ANTI_OVERHANG, SUPPORT_DROP_DOWN, SUPPORT_VANILLA };
		//ModifierType modifier_type = (anti_overhang_mesh) ? ANTI_OVERHANG : (support_mesh_drop_down) ? SUPPORT_DROP_DOWN : SUPPORT_VANILLA);
		for (unsigned int layer_nr = 0; layer_nr < slicer->layers.size(); layer_nr++)
		{
			SupportLayer& support_layer = storage.support.supportLayers[layer_nr];
			const SlicerLayer& slicer_layer = slicer->layers[layer_nr];
			support_layer.support_mesh_drop_down.add(slicer_layer.polygons);	  //Support Mesh down
	...
	//	const Settings& mesh_group_settings = Application::getInstance().current_slice->scene.current_mesh_group->settings;
		//const ExtruderTrain& infill_extruder = 0;// mesh_group_settings.get<ExtruderTrain&>("support_infill_extruder_nr");
		const EFillMethod support_pattern = EFillMethod::ZIG_ZAG;
		const coord_tIrfan support_line_width = MM2INT(0.35);// coord_tIrfan > ("support_line_width");

		// the wall line count is used for calculating insets, and we generate support infill patterns within the insets
		const size_t wall_line_count = 0;// infill_extruder.settings.get<size_t>("support_wall_count");
	...
	void AreaSupport::generateGradualSupport(SliceDataStorage& storage)
	{
		...
		const coord_tIrfan gradual_support_step_height = MM2INT(1);// infill_extruder.settings.get<coord_tIrfan>("gradual_support_infill_step_height");
		const size_t max_density_steps = 0;// infill_extruder.settings.get<size_t>("gradual_support_infill_steps");
		...
		size_t gradual_support_step_layer_count = MM2INT(0.01 * 15);// round_divide(gradual_support_step_height, mesh_group_settings.get<coord_tIrfan>("layer_height")); //The difference in layer count between consecutive density infill areas.
		...
		int  min_layer = 0;
		int  max_layer = total_layer_count - 1;

		// compute different density areas for each support island
		for (int layer_nr = 0; layer_nr < static_cast<int>(total_layer_count) - 1; layer_nr++)
		{
			...
			for (unsigned int part_idx = 0; part_idx < support_infill_parts.size(); ++part_idx)
			{
				...
				{
					int min_layer = layer_nr + density_step * gradual_support_step_layer_count + int(layer_skip_count);
					int max_layer = layer_nr + (density_step + 1) * gradual_support_step_layer_count;
	...
	void AreaSupport::combineSupportInfillLayers(SliceDataStorage& storage)
	{
		...
		const coord_tIrfan layer_height = storage.getlayer_thickness();// MM2INT(0.01);// mesh_group_settings.get<coord_tIrfan>("layer_height");
		// How many support infill layers to combine to obtain the requested sparse thickness.
		//const ExtruderTrain& infill_extruder = mesh_group_settings.get<ExtruderTrain&>("support_infill_extruder_nr");
		const size_t combine_layers_amount = std::max(1U, round_divide(MM2INT(0.2), std::max(layer_height, coord_tIrfan(1))));
	...
	void AreaSupport::generateOutlineInsets(std::vector<Polygons>& insets, Polygons& outline, const unsigned int inset_count, const coord_tIrfan wall_line_width_x)
	{
	...
	{
		const coord_tIrfan support_line_width = MM2INT(0.35);// Application::getInstance().current_slice->scene.current_mesh_group->settings.get<coord_tIrfan>("support_line_width");
	...
	/*
	Polygons AreaSupport::join(const SliceDataStorage& storage, const Polygons& supportLayer_up, Polygons& supportLayer_this, const coord_tIrfan smoothing_distance)
	{
		Polygons joined;
		...
		const double conical_support_angle = 0.52;// infill_settings.get<AngleRadians>("support_conical_angle");
		const coord_tIrfan layer_thickness = MM2INT(0.01); //infill_settings.get<coord_tIrfan>("layer_height");
		coord_tIrfan conical_support_offset;
		...
		bool support_conical_enabled = false;
		const bool conical_support = support_conical_enabled  && conical_support_angle != 0;
		joined = supportLayer_this.unionPolygons(supportLayer_up);
		
		// join different parts
		const coord_tIrfan join_distance = MM2INT(2.0);// infill_settings.get<coord_tIrfan>("support_join_distance");
		if (join_distance > 0)
		{
		...
		const double max_smoothing_angle = 135; // maximum angle of inner corners to be smoothed
   ...
   */
	void AreaSupport::generateOverhangAreas(SliceDataStorage& storage)
	{
		for (SliceMeshStorage& mesh : storage.meshes)
		{
			...
			bool anti_overhang_mesh = false;
			if (infill_mesh || anti_overhang_mesh)
			{
				continue;
			}
	...
	void AreaSupport::generateSupportAreas(SliceDataStorage& storage)
	{
		...
		for (unsigned int mesh_idx = 0; mesh_idx < storage.meshes.size(); mesh_idx++)
		{
			...
			if (support_mesh)
			{
				...
				if ((support_mesh_drop_down && support_meshes_drop_down_handled) ||
					(!support_mesh_drop_down && support_meshes_handled))
			...
			generateSupportAreasForMesh(storage, mesh_idx, storage.print_layer_count, mesh_support_areas_per_layer);
			const double minimum_support_area = 0.0;// mesh.settings.get<double>("minimum_support_area");
	...
	void AreaSupport::precomputeCrossInfillTree(SliceDataStorage& storage)
	{
		//const Settings& mesh_group_settings = Application::getInstance().current_slice->scene.current_mesh_group->settings;
		//const ExtruderTrain& infill_extruder = mesh_group_settings.get<ExtruderTrain&>("support_infill_extruder_nr");
		const EFillMethod& support_pattern = EFillMethod::ZIG_ZAG;// infill_extruder.settings.get<EFillMethod>("support_pattern");
		/*
		if (support_pattern == EFillMethod::CROSS || support_pattern == EFillMethod::CROSS_3D)
		{
			...
			for (unsigned int mesh_idx = 0; mesh_idx < storage.meshes.size(); mesh_idx++)
			{
				...
				const coord_tIrfan aabb_expansion = infill_settings.get<coord_tIrfan>("support_offset");
			...
			{
				storage.support.cross_fill_provider = new SierpinskiFillProvider(aabb, infill_extruder.settings.get<coord_tIrfan>("support_line_distance"), infill_extruder.settings.get<coord_tIrfan>("support_line_width"), cross_subdisivion_spec_image_file);
			}
			else
			{
				...
				storage.support.cross_fill_provider = new SierpinskiFillProvider(aabb, infill_extruder.settings.get<coord_tIrfan>("support_line_distance"), infill_extruder.settings.get<coord_tIrfan>("support_line_width"));
			}
		}
		*/
	}
	...
	//Fill the overhang areas with emptiness first, even if it's a support mesh, so that we can request the areas.
		...
		const bool is_support_modifier = support_mesh;// mesh.settings.get<bool>("support_mesh");
		...
		const coord_tIrfan layer_height = MM2INT(0.01*15);// Application::getInstance().current_slice->scene.current_mesh_group->settings.get<coord_tIrfan>("layer_height");
		const coord_tIrfan z_distance_top = MM2INT(0.4);// mesh.settings.get<coord_tIrfan>("support_top_distance");
		...
		const coord_tIrfan max_supported_diameter = MM2INT(3.0);// mesh.settings.get<coord_tIrfan>("support_tower_maximum_supported_diameter");
		
		const bool use_towers = true;// mesh.settings.get<bool>("support_use_towers") && max_supported_diameter > 0;
		if (use_towers)
		{
...
// Use a signed type for the loop counter so MSVC compiles (because it uses OpenMP 2.0, an old version).
	...
	*/
	 ...
	 */
	void AreaSupport::generateSupportAreasForMesh(SliceDataStorage& storage, int mesh_idx, const size_t layer_count, std::vector<Polygons>& support_areas)
	{
		SliceMeshStorage& mesh = storage.meshes[mesh_idx];
		bool support_roof_enable = false;
		bool support_bottom_enable = false;
		...
		const bool is_support_mesh_place_holder = false; // whether this mesh has empty SliceMeshStorage and this function is now called to only generate support for all support meshes

		bool support_enable = false;

		if (!support_enable && !is_support_mesh_place_holder)
		{
			return;
		}
	}
	/*
		//const Settings& mesh_group_settings = Application::getInstance().current_slice->scene.current_mesh_group->settings;
		// support type is build plate const ESupportType support_type = mesh_group_settings.get<ESupportType>("support_type");
		// early out
		const coord_tIrfan layer_thickness = MM2INT(0.01 * 15);// "layer_height");
		const coord_tIrfan z_distance_top = MM2INT(0.4);// "support_top_distance");
		...
		const coord_tIrfan xy_distance = MM2INT(0.875);// infill_settings.get<coord_tIrfan>("support_xy_distance");
		const coord_tIrfan xy_distance_overhang = MM2INT(0.35);// infill_settings.get<coord_tIrfan>("support_xy_distance_overhang");
		const bool use_xy_distance_overhang = true;// infill_settings.get<SupportDistPriority>("support_xy_overrides_z") == SupportDistPriority::Z_OVERRIDES_XY; // whether to use a different xy distance at overhangs
		const double angle = 0.872665;// ((mesh.settings.get<bool>("support_roof_enable")) ? roof_settings : infill_settings).get<AngleRadians>("support_angle");
...
// Use a signed type for the loop counter so MSVC compiles (because it uses OpenMP 2.0, an old version).
		...
		const bool is_support_mesh_nondrop_place_holder = false;// is_support_mesh_place_holder && !mesh.settings.get<bool>("support_mesh_drop_down");
		const bool is_support_mesh_drop_down_place_holder = false;// is_support_mesh_place_holder && mesh.settings.get<bool>("support_mesh_drop_down");

		const coord_tIrfan bottom_stair_step_width = std::max(static_cast<coord_tIrfan>(0), MM2INT(5.0));
		const coord_tIrfan extension_offset = MM2INT(0.2);// infill_settings.get<coord_tIrfan>("support_offset");

		const coord_tIrfan max_tower_supported_diameter = MM2INT(2.0);// infill_settings.get<coord_tIrfan>("support_tower_maximum_supported_diameter");
		const bool use_towers = true;// infill_settings.get<bool>("support_use_towers") && max_tower_supported_diameter > 0;

		coord_tIrfan smoothing_distance;
		{ // compute best smoothing_distance
			...
			const coord_tIrfan infill_line_width = MM2INT(0.35);// infill_train.settings.get<coord_tIrfan>("support_line_width");
			...
			if (support_roof_enable)
			{
				...
				const coord_tIrfan roof_line_width = MM2INT(0.35);// roof_train.settings.get<coord_tIrfan>("support_roof_line_width");
			...
			if (support_bottom_enable)
			{
				...
				const coord_tIrfan bottom_line_width = MM2INT(0.35);// bottom_train.settings.get<coord_tIrfan>("support_bottom_line_width");
				smoothing_distance = std::max(smoothing_distance, bottom_line_width);
			}
		}

		const coord_tIrfan z_distance_bottom = MM2INT(0.2);// ((mesh.settings.get<bool>("support_bottom_enable")) ? bottom_settings : infill_settings).get<coord_tIrfan>("support_bottom_distance");
		...
		const coord_tIrfan bottom_stair_step_height = MM2INT(0.3);// std::max(static_cast<coord_tIrfan>(0), mesh.settings.get<coord_tIrfan>("support_bottom_stair_step_height"));
		...
		for (size_t layer_idx = layer_count - 1 - layer_z_distance_top; layer_idx != static_cast<size_t>(-1); layer_idx--)
		{
			...
			if (use_towers && !is_support_mesh_place_holder)
			{
				...
				AreaSupport::handleWallStruts(layer_this);
				// handle towers
				AreaSupport::handleTowers(layer_this, tower_roofs, mesh.overhang_points, layer_idx, layer_count);
			}
				...
				for (PolygonsPart poly : layer_this.splitIntoParts())
				{
					...
					if (part_area > 0 && part_area < max_tower_supported_diameter * max_tower_supported_diameter)
					{
						...
						if (layer_idx < layer_count - tower_top_layer_count && layer_idx >= tower_top_layer_count + bottom_empty_layer_count)
						{
							...
							const curaIrfan::PointIrfan middle = AABB(poly).getMiddle();
							const bool has_support_above = layer_above.inside(middle);
							constexpr bool no_support = false;
							constexpr bool no_prime_tower = false;
							const bool has_model_below = false;// torage.getLayerOutlines(layer_idx - tower_top_layer_count - bottom_empty_layer_count, no_support, no_prime_tower).inside(middle);
...
#pragma omp parallel for default(none) shared(support_areas, storage) schedule(dynamic)
	...
	*/
	...
	void AreaSupport::moveUpFromModel(const SliceDataStorage& storage, Polygons& stair_removal, Polygons& support_areas, const size_t layer_idx, const size_t bottom_empty_layer_count, const size_t bottom_stair_step_layer_count, const coord_tIrfan support_bottom_stair_step_width)
	{
	...
	{
		Polygons supportLayer_supportee = storage.Layers[layer_idx].getOutlines();
		constexpr bool no_support = false;
		constexpr bool no_prime_tower = false;
		Polygons supportLayer_supporter = storage.getLayerOutlines(layer_idx - 1, no_support, no_prime_tower);

		const coord_tIrfan layer_height = MM2INT(0.01 * 15);// mesh.settings.get<coord_tIrfan>("layer_height");
		const double support_angle = 1.0472;// mesh.settings.get<AngleRadians>("support_angle");
		const double tan_angle = tan(support_angle) - 0.01;  //The X/Y component of the support angle. 0.01 to make 90 degrees work too.
		const coord_tIrfan max_dist_from_lower_layer = tan_angle * layer_height; //Maximum horizontal distance that can be bridged.
	...
	void AreaSupport::detectOverhangPoints(const SliceDataStorage& storage, SliceMeshStorage& mesh)
	{
		...
		const coord_tIrfan support_line_width = MM2INT(0.35);// infill_extruder.settings.get<coord_tIrfan>("support_line_width");
		const coord_tIrfan max_tower_supported_diameter = MM2INT(2.0);// mesh.settings.get<coord_tIrfan>("support_tower_maximum_supported_diameter");
		...
		{
			const SliceLayer& layer = storage.Layers[layer_idx];
			for (const SliceLayerPart& part : layer.parts)
			{
				if (part.outline.outerPolygon().area() < max_tower_supported_diameter * max_tower_supported_diameter)
				{
					const SliceLayer& layer_below = storage.Layers[layer_idx - 1];
	...
	{
		int layer_overhang_point = layer_idx + 1; //Start tower 1 layer below overhang point.
		if (layer_overhang_point >= static_cast<int>(layer_count) - 1)
		{
			return;
		}
		...
		if (overhang_points_here.size() > 0)
		{
			{ // make sure we have the lowest point (make polys empty if they have small parts below)
				if (layer_overhang_point < static_cast<int>(layer_count) && !overhang_points[layer_overhang_point - 1].empty())
				{
					const coord_tIrfan max_tower_supported_diameter = MM2INT(2.0);// settings.get<coord_tIrfan>("support_tower_maximum_supported_diameter");
		...
		// make tower roofs
		const coord_tIrfan layer_thickness = MM2INT(0.01 * 15);// settings.get<coord_tIrfan>("layer_height");
		const double tower_roof_angle = 1.0472;// settings.get<AngleRadians>("support_tower_roof_angle");
		const double tan_tower_roof_angle = tan(tower_roof_angle);
		const coord_tIrfan tower_roof_expansion_distance = layer_thickness / tan_tower_roof_angle;
		const coord_tIrfan tower_diameter = MM2INT(3.0);// settings.get<coord_tIrfan>("support_tower_diameter");
	...
	void AreaSupport::handleWallStruts(Polygons& supportLayer_this)
	{
		const coord_tIrfan max_tower_supported_diameter = MM2INT(2.0);// settings.get<coord_tIrfan>("support_tower_maximum_supported_diameter");
		const coord_tIrfan tower_diameter = MM2INT(3.0);// settings.get<coord_tIrfan>("support_tower_diameter");
		for (unsigned int p = 0; p < supportLayer_this.size(); p++)
		{
			...
			if (poly.size() < 6) // might be a single wall
			{
				...
				{
					int length2 = curaIrfan::vSize2(curaIrfan::operator-(poly[i],poly[(i + 1) % poly.size()]));
				...
				{
					curaIrfan::PointIrfan mid = curaIrfan::operator/((curaIrfan::operator+(poly[best], poly[(best + 1) % poly.size()])), 2);
					Polygons struts;
					PolygonRef strut = struts.newPoly();
					strut.add(curaIrfan::operator+(mid, curaIrfan::PointIrfan(tower_diameter / 2, tower_diameter / 2)));
					strut.add(curaIrfan::operator+(mid, curaIrfan::PointIrfan(-tower_diameter / 2, tower_diameter / 2)));
					strut.add(curaIrfan::operator+(mid, curaIrfan::PointIrfan(-tower_diameter / 2, -tower_diameter / 2)));
					strut.add(curaIrfan::operator+(mid, curaIrfan::PointIrfan(tower_diameter / 2, -tower_diameter / 2)));
	...
	void AreaSupport::generateSupportBottom(SliceDataStorage& storage, const SliceMeshStorage& mesh, std::vector<Polygons>& global_support_areas_per_layer)
	{
		...
		const size_t bottom_layer_count = round_divide(1, layer_height); //Number of layers in support bottom.
		if (bottom_layer_count <= 0)
		{
			return;
		}
		const coord_tIrfan z_distance_bottom = round_up_divide(MM2INT(0.2), layer_height); //Number of layers between support bottom and model.
		const size_t skip_layer_count = std::max(1u, round_divide(MM2INT(0.3), layer_height)); //Resolution of generating support bottoms above model.
		const coord_tIrfan bottom_line_width = MM2INT(0.35);// mesh_group_settings.get<ExtruderTrain&>("support_bottom_extruder_nr").settings.get<coord_tIrfan>("support_bottom_line_width");
		const coord_tIrfan bottom_outline_offset = MM2INT(0.0);// mesh_group_settings.get<ExtruderTrain&>("support_bottom_extruder_nr").settings.get<coord_tIrfan>("support_bottom_offset");
		...
		const double minimum_bottom_area = 1.0;
		...
		for (unsigned int layer_idx = z_distance_bottom; layer_idx < support_layers.size(); layer_idx++)
		{
			...
			{
				mesh_outlines.add(storage.Layers[std::round(layer_idx_below)].getOutlines());
			}
			Polygons bottoms;
	...
	void AreaSupport::generateSupportRoof(SliceDataStorage& storage, const SliceMeshStorage& mesh, std::vector<Polygons>& global_support_areas_per_layer)
	{
		//const Settings& mesh_group_settings = Application::getInstance().current_slice->scene.current_mesh_group->settings;
		const coord_tIrfan layer_height = MM2INT(0.01 * 15);// mesh_group_settings.get<coord_tIrfan>("layer_height");
		const size_t roof_layer_count = round_divide(1, layer_height); //Number of layers in support roof.
		if (roof_layer_count <= 0)
		{
			return;
		}
		const coord_tIrfan z_distance_top = round_up_divide(MM2INT(0.4), layer_height); //Number of layers between support roof and model.
		const size_t skip_layer_count = std::max(1u, round_divide(MM2INT(0.3), layer_height)); //Resolution of generating support roof below model.
		const coord_tIrfan roof_line_width = MM2INT(0.35);// mesh_group_settings.get<ExtruderTrain&>("support_roof_extruder_nr").settings.get<coord_tIrfan>("support_roof_line_width");
		const coord_tIrfan roof_outline_offset = MM2INT(0.0);// mesh_group_settings.get<ExtruderTrain&>("support_roof_extruder_nr").settings.get<coord_tIrfan>("support_roof_offset");
		...
		const double minimum_roof_area = 1.0;

		std::vector<SupportLayer>& support_layers = storage.support.supportLayers;
		for (int layer_idx = 0; layer_idx < static_cast<int>(support_layers.size() - z_distance_top); layer_idx++)
		{
			const int top_layer_idx_above =static_cast<int>(support_layers.size() - 1); //Maximum layer of the model that generates support roof.
			Polygons mesh_outlines;
			for (float layer_idx_above = top_layer_idx_above; layer_idx_above > layer_idx + z_distance_top; layer_idx_above -= z_skip)
			{
				mesh_outlines.add(storage.Layers[std::round(layer_idx_above)].getOutlines());
			}
			Polygons roofs;
	...
	void AreaSupport::generateSupportInterfaceLayer(Polygons& support_areas, const Polygons colliding_mesh_outlines, const coord_tIrfan safety_offset, const coord_tIrfan outline_offset, const double minimum_interface_area, Polygons& interface_polygons)
	{
