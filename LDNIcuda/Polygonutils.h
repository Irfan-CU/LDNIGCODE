...
/*!
	...
	{
		curaIrfan::PointIrfan location; //!< Result location
		...
		ClosestPolygonPoint(curaIrfan::PointIrfan p, int pos, ConstPolygonRef poly) : location(p), poly(poly), poly_idx(NO_INDEX), point_idx(pos) {};
		ClosestPolygonPoint(curaIrfan::PointIrfan p, int pos, ConstPolygonRef poly, int poly_idx) : location(p), poly(poly), poly_idx(poly_idx), point_idx(pos) {};
...
{
	template <>
	struct hash<ClosestPolygonPoint>
	{
		size_t operator()(const ClosestPolygonPoint& cpp) const
		{
			return std::hash<curaIrfan::PointIrfan>()(cpp.p());
		}
	};
}//namespace std
...
//namespace std
	...
	{
		std::pair<curaIrfan::PointIrfan, curaIrfan::PointIrfan> operator()(const PolygonsPointIndex& val) const
		{
			ConstPolygonRef poly = (*val.polygons)[val.poly_idx];
			curaIrfan::PointIrfan start = poly[val.point_idx];
			unsigned int next_point_idx = (val.point_idx + 1) % poly.size();
			curaIrfan::PointIrfan end = poly[next_point_idx];
			return std::pair<curaIrfan::PointIrfan, curaIrfan::PointIrfan>(start, end);
		}
	};
	...
	{
	public:
		static const std::function<int(curaIrfan::PointIrfan)> no_penalty_function; //!< Function always returning zero
		 ...
		 */
		static curaIrfan::PointIrfan getVertexInwardNormal(ConstPolygonRef poly, unsigned int point_idx);
		...
		*/
		static curaIrfan::PointIrfan getBoundaryPointWithOffset(ConstPolygonRef poly, unsigned int point_idx, int64_t offset);
		 ...
		 */
		static curaIrfan::PointIrfan moveInsideDiagonally(ClosestPolygonPoint point_on_boundary, int64_t inset);
		...
		*/
		static unsigned int moveInside(const Polygons& polygons, curaIrfan::PointIrfan& from, int distance = 0, int64_t max_dist2 = std::numeric_limits<int64_t>::max());
		 ...
		 */
		static unsigned int moveInside(const ConstPolygonRef polygon, curaIrfan::PointIrfan& from, int distance = 0, int64_t max_dist2 = std::numeric_limits<int64_t>::max());
		 ...
		 */
		static ClosestPolygonPoint moveInside2(const Polygons& polygons, curaIrfan::PointIrfan& from, const int distance = 0, const int64_t max_dist2 = std::numeric_limits<int64_t>::max(), const Polygons* loc_to_line_polygons = nullptr, const LocToLineGrid* loc_to_line_grid = nullptr, const std::function<int(curaIrfan::PointIrfan)>& penalty_function = no_penalty_function);
		 ...
		 * \return The point on the polygon closest to \p from
		 */
		static ClosestPolygonPoint moveInside2(const Polygons& loc_to_line_polygons, ConstPolygonRef polygon, curaIrfan::PointIrfan& from, const int distance = 0, const int64_t max_dist2 = std::numeric_limits<int64_t>::max(), const LocToLineGrid* loc_to_line_grid = nullptr, const std::function<int(curaIrfan::PointIrfan)>& penalty_function = no_penalty_function);
		 ...
		 * \return The index to the polygon onto which we have moved the point.
		 */
		static unsigned int moveOutside(const Polygons& polygons, curaIrfan::PointIrfan& from, int distance = 0, int64_t max_dist2 = std::numeric_limits<int64_t>::max());
		 ...
		 */
		static curaIrfan::PointIrfan moveInside(const ClosestPolygonPoint& cpp, const int distance);
		 ...
		 * \return A point at a \p distance from the point in \p cpp orthogonal to the boundary there.
		 */
		static curaIrfan::PointIrfan moveOutside(const ClosestPolygonPoint& cpp, const int distance);
		 ...
		 * \return The point on the polygon closest to \p from
		 */
		static ClosestPolygonPoint ensureInsideOrOutside(const Polygons& polygons, curaIrfan::PointIrfan& from, int preferred_dist_inside, int64_t max_dist2 = std::numeric_limits<int64_t>::max(), const Polygons* loc_to_line_polygons = nullptr, const LocToLineGrid* loc_to_line_grid = nullptr, const std::function<int(curaIrfan::PointIrfan)>& penalty_function = no_penalty_function);
		 ...
		 * \return The point on the polygon closest to \p from
		 */
		static ClosestPolygonPoint ensureInsideOrOutside(const Polygons& polygons, curaIrfan::PointIrfan& from, const ClosestPolygonPoint& closest_polygon_point, int preferred_dist_inside, const Polygons* loc_to_line_polygons = nullptr, const LocToLineGrid* loc_to_line_grid = nullptr, const std::function<int(curaIrfan::PointIrfan)>& penalty_function = no_penalty_function);
		 ...
		 */
		static std::pair<ClosestPolygonPoint, ClosestPolygonPoint> findConnection(ConstPolygonRef poly1, Polygons& polys2, coord_tIrfan min_connection_length, coord_tIrfan max_connection_length, std::function<bool(std::pair<ClosestPolygonPoint, ClosestPolygonPoint>)> precondition);
		...
		*/
		static ClosestPolygonPoint findNearestClosest(curaIrfan::PointIrfan from, ConstPolygonRef polygon, int start_idx);
		...
		*/
		static ClosestPolygonPoint findNearestClosest(const curaIrfan::PointIrfan from, ConstPolygonRef polygon, int start_idx, int direction);
		 ...
		 * \param penalty_function A function returning a penalty term on the squared distance score of a candidate point.
		 */
		static ClosestPolygonPoint findClosest(curaIrfan::PointIrfan from, const Polygons& polygons, const std::function<int(curaIrfan::PointIrfan)>& penalty_function = no_penalty_function);
		 ...
		 * \param penalty_function A function returning a penalty term on the squared distance score of a candidate point.
		 */
		static ClosestPolygonPoint findClosest(curaIrfan::PointIrfan from, ConstPolygonRef polygon, const std::function<int(curaIrfan::PointIrfan)>& penalty_function = no_penalty_function);
		 ...
		 */
		static PolygonsPointIndex findNearestVert(const curaIrfan::PointIrfan from, const Polygons& polys);
		 ...
		 */
		static unsigned int findNearestVert(const curaIrfan::PointIrfan from, ConstPolygonRef poly);
		 ...
		 */
		static std::optional<ClosestPolygonPoint> findClose(curaIrfan::PointIrfan from, const Polygons& polygons, const LocToLineGrid& loc_to_line, const std::function<int(curaIrfan::PointIrfan)>& penalty_function = no_penalty_function);
		 ...
		 */
		static std::vector<std::pair<ClosestPolygonPoint, ClosestPolygonPoint>> findClose(ConstPolygonRef from, const Polygons& destination, const LocToLineGrid& destination_loc_to_line, const std::function<int(curaIrfan::PointIrfan)>& penalty_function = no_penalty_function);
		 ...
		 */
		static bool polygonCollidesWithLineSegment(const curaIrfan::PointIrfan from, const curaIrfan::PointIrfan to, const LocToLineGrid& loc_to_line ,PolygonsPointIndex* collision_result = nullptr);
		...
		*/
		static bool getNextPointWithDistance(curaIrfan::PointIrfan from, int64_t dist, ConstPolygonRef poly, int start_idx, int poly_start_idx, GivenDistPoint& result);
		 ...
		 */
		static std::optional<ClosestPolygonPoint> getNextParallelIntersection(const ClosestPolygonPoint& start, const curaIrfan::PointIrfan& line_to, const coord_tIrfan dist, const bool forward);
		 ...
		 * polygon(s)
		 */
		static bool polygonCollidesWithLineSegment(ConstPolygonRef poly, const curaIrfan::PointIrfan& transformed_startPoint, const curaIrfan::PointIrfan& transformed_endPoint, curaIrfan::PointMatrix transformation_matrix);
		 ...
		 * polygon(s)
		 */
		static bool polygonCollidesWithLineSegment(ConstPolygonRef poly, const curaIrfan::PointIrfan& startPoint, const curaIrfan::PointIrfan& endPoint);
		 ...
		 * polygon(s)
		 */
		static bool polygonCollidesWithLineSegment(const Polygons& polys, const curaIrfan::PointIrfan& transformed_startPoint, const curaIrfan::PointIrfan& transformed_endPoint, curaIrfan::PointMatrix transformation_matrix);
		 ...
		 * polygon(s)
		 */
		static bool polygonCollidesWithLineSegment(const Polygons& polys, const curaIrfan::PointIrfan& startPoint, const curaIrfan::PointIrfan& endPoint);
		 ...
		 */
		static bool polygonOutlinesAdjacent(const ConstPolygonRef inner_poly, const ConstPolygonRef outer_poly, const coord_tIrfan max_gap);
		 ...
		 * \param[in] max_gap Polygons must be closer together than this distance to be considered adjacent.
		 */
		static void findAdjacentPolygons(std::vector<unsigned>& adjacent_poly_indices, const ConstPolygonRef& poly, const std::vector<ConstPolygonPointer>& possible_adjacent_polys, const coord_tIrfan max_gap);
	...
	private:
		 ...
		 * \return The point on the polygon closest to \p from
		 */
		static ClosestPolygonPoint _moveInside2(const ClosestPolygonPoint& closest_polygon_point, const int distance, curaIrfan::PointIrfan& from, const int64_t max_dist2);
