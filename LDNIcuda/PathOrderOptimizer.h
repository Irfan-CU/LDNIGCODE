...
#include "Polygonutils.h"
	...
	public:
		EZSeamType type;
		curaIrfan::PointIrfan pos; //!< The position near where to create the z_seam (if \ref PathOrderOptimizer::type == 'back')
		EZSeamCornerPrefType corner_pref;
		// default constructor
		ZSeamConfig()
			: type(EZSeamType::SHORTEST)
			, pos(curaIrfan::PointIrfan(0, 0))
			, corner_pref(EZSeamCornerPrefType::Z_SEAM_CORNER_PREF_NONE)
		{
		}
		ZSeamConfig(EZSeamType type, curaIrfan::PointIrfan pos, EZSeamCornerPrefType corner_pref)
			: type(type)
			, pos(pos)
	...
	{
	public:
		curaIrfan::PointIrfan startPoint; //!< A location near the prefered start location
		...
		PathOrderOptimizer(curaIrfan::PointIrfan startPoint, const ZSeamConfig config = ZSeamConfig(), const Polygons* combing_boundary = nullptr)
	...
	private:
		int getClosestPointInPolygon(curaIrfan::PointIrfan prev, int i_polygon); //!< returns the index of the closest point
	...
	*/
	...
	{
	public:
		curaIrfan::PointIrfan startPoint; //!< The location of the nozzle before starting to print the current layer
		std::vector<ConstPolygonPointer> polygons; //!< the parts of the layer (in arbitrary order)
		std::vector<int> polyStart; //!< polygons[i][polyStart[i]] = point of polygon i which is to be the starting point in printing the polygon
		std::vector<int> polyOrder; //!< the optimized order as indices in #polygons
		...
		LineOrderOptimizer(curaIrfan::PointIrfan startPoint, const Polygons* combing_boundary = nullptr)
		{
	...
	private:
		 ...
		 */
		void updateBestLine(unsigned int poly_idx, int& best, float& best_score, curaIrfan::PointIrfan prev_point, int just_point = -1);
		 ...
		 * \return The squared travel distance between the two points
		 */
		float combingDistance2(const curaIrfan::PointIrfan &p0, const curaIrfan::PointIrfan &p1);
	};
