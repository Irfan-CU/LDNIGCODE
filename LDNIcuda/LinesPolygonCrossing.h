...
#include "PolygonUtils.h"
	...
	private:
		...
		struct Crossing
		{
			coord_tIrfan x; //!< x coordinate of crossings between the polygon and the scanline.
			 ...
			 */
			Crossing(const coord_tIrfan x, const size_t point_idx);
		};
		...
		curaIrfan::PointIrfan startPoint; //!< The start point of the scanline.
		curaIrfan::PointIrfan endPoint; //!< The end point of the scanline.
		...
		curaIrfan::PointMatrix transformation_matrix; //!< The transformation which rotates everything such that the scanline is aligned with the x-axis.
		curaIrfan::PointIrfan transformed_startPoint; //!< The LinePolygonsCrossings::startPoint as transformed by Comb::transformation_matrix such that it has (roughly) the same Y as transformed_endPoint
		curaIrfan::PointIrfan transformed_endPoint; //!< The LinePolygonsCrossings::endPoint as transformed by Comb::transformation_matrix such that it has (roughly) the same Y as transformed_startPoint
		 ...
		 */
		LinePolygonsCrossings(const Polygons& boundary, LocToLineGrid& loc_to_line_grid, curaIrfan::PointIrfan& start, curaIrfan::PointIrfan& end, int64_t dist_to_move_boundary_point_outside)
			: boundary(boundary)
	...
	public:
		 ...
		 * \return Whether combing succeeded, i.e. we didn't cross any gaps/other parts
		 */
		static bool comb(const Polygons& boundary, LocToLineGrid& loc_to_line_grid, curaIrfan::PointIrfan startPoint, curaIrfan::PointIrfan endPoint, CombPath& combPath, int64_t dist_to_move_boundary_point_outside, int64_t max_comb_distance_ignored, bool fail_on_unavoidable_obstacles)
		{
